...
PlayerEvents.tick(function(event) {
  ...
  if (player.mainHandItem && player.mainHandItem.id === "mupt:tier_meter") hasMeter = true;
  if (!hasMeter && player.offhandItem && player.offhandItem.id === "mupt:tier_meter") hasMeter = true;
  if (!hasMeter) {
    for (var s = 0; s < inv.containerSize; s++) {
      var slotItem = inv.getItem(s);
      if (slotItem && slotItem.id === "mupt:tier_meter") {
        hasMeter = true;
        break;
      }
    }
  ...
  function applyIfMeter(stack) {
    if (!stack || stack.id !== "mupt:tier_meter") return;
